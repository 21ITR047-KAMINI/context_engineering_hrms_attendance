# ==========================================
# Leave Agent (Prompt-driven - FINAL)
# ==========================================
...
def create_leave_agent(llm, tools):
    ...
    def agent_node(state):
        try:
            ...
            print(f"\n[LEAVE AGENT] Query: {query}")

            # ----------------------------------
            # STEP 1: BUILD PROMPT (CORE FIX)
            # ----------------------------------
            prompt = f"""
You are an expert HR leave management SQL assistant.
...
COLUMN MEANING:
- leave_status → leave type/status (H = Half-day, L = Leave, etc.)
- leave_remark → reason for leave
- Leave_type → category of leave
- LopCount → loss of pay (impact)

BUSINESS RULES:
- Always use leave_status and leave_remark to understand leave
- Use LopCount to detect payroll impact
- DO NOT use login_mast for leave queries
- Focus only on leave-related data
